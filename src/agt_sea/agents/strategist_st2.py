"""
agt_sea — Strategist Agent (Standard 2.0)

Takes a raw client brief and produces a structured creative brief
that will guide the Standard 2.0 Creative A agent's territory
generation.
"""
...
def run_strategist_st2(state: AgencyState) -> AgencyState:
    ...
    messages = [
        SystemMessage(
            content=_build_system_prompt(state.strategist_st2_strategic_philosophy)
        ),
    ...
    state.history.append(
        AgentOutput(
            agent=AgentRole.STRATEGIST_ST2,
            provider=provider,
            model=model,
