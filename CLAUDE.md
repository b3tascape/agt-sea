...
- Three philosophy fields on `AgencyState`: `strategic_philosophy` (Strategist), `creative_philosophy` (Creative), `cd_philosophy` (Creative Director). All default to `NEUTRAL`. Sidebar mirrors to `st.session_state`; every page that builds `AgencyState` must pass all three through
...
- **Per-role lenses on `AgencyState`** — provenance + taste scoped per *creative role*: `creative1_*`, `creative2_*`, and shared `cd_*` (CD Feedback + CD Synthesis). All default to `NEUTRAL`. CD Grader is always neutral by contract — never inject philosophy/provenance/taste into it
- **Per-agent temperature on `AgencyState`** — `creative1_temperature`, `creative2_temperature`, `cd_feedback_temperature`, `cd_synthesis_temperature` (default `0.7`); `grader_temperature` (default `0.0`, not sidebar-exposed). All bounded `0.0–1.0`. Lives in the `# --- Iteration tracking ---` block
...
- **Philosophy injection pattern (Standard 1.0).** Strategist, Creative, and Creative Director each build their system prompt via `_build_system_prompt(philosophy)` (Creative also has `_build_revision_prompt`). Each follows the neutral-skip rule. Strategist reads `state.strategic_philosophy`, Creative reads `state.creative_philosophy`, CD reads `state.cd_philosophy` (not `creative_philosophy`)
- Standard 2.0 agent helpers (`_build_system_prompt(...)` shapes per agent) are detailed in **V2 Workflow Conventions** above
- **Validation retry helper is shared.** `invoke_with_validation_retry` lives in `agt_sea/llm/provider.py`, generic over any Pydantic model. Use it (not the structured runnable directly) in every agent that uses `with_structured_output()` — Creative 1, Creative 2, Creative Director, CD Grader, CD Synthesis
...
- `uv run pytest tests/` runs unit tests only — `pyproject.toml` ignores manual real-LLM integration scripts (`test_strategist`, `test_creative`, `test_creative1`, `test_creative2`, `test_pipeline`, `test_pipeline_v2`). Run those directly with `uv run python tests/test_<name>.py`
...
- **Sidebar scope** — top-level controls shared by 1.0 and 2.0 (philosophies, provider, model, max iterations, approval threshold). Standard 2.0 controls (per-role provenance + taste, per-agent temperature) live in a single collapsed `st.sidebar.expander("STANDARD 2.0 CONTROLS")`. CD Grader is not sidebar-exposed: neutral and temp-0 by contract
- **Creative page tabs** — `frontend/pages/creative.py` has two tabs: `c1_territory` (default) runs Creative 1 with `render_territory_cards()`; `c0_original` runs Standard 1.0's creative agent unchanged. Each tab owns its own session-state keys (`c1_brief_input`/`c1_result` vs `creative_brief_input`/`creative_result`). `num_territories` (1–12) is per-page on `c1_territory`, not sidebar-exposed
- **Workflow page tabs** — `frontend/pages/workflow.py` has two tabs: `Standard 2.0` (default, runs v2 graph) and `Standard 1.0` (lifted verbatim into `_render_standard_v1()` — do not edit when extending v2). Each tab owns its own session-state keys
  ...
  - **Extended components.** `progress.py` carries v2 node labels/previews (compact for `creative2`/`cd_feedback`/`cd_synthesis`; full audit detail lives in pipeline history at terminal). `history.py` handles every `AgentRole` (v1 + `CREATIVE_1`, `CREATIVE_2`, `CD_GRADER`, `CD_FEEDBACK`, `CD_SYNTHESIS`) with iteration counters scoped per-role. `synthesis_output.py` is the v2-only terminal renderer for `CDSynthesis` + `CampaignConcept`. `territory_cards.py` exposes both `render_territory_cards()` (bordered grid) and `render_territory_body()` (boundary-free, reused by the selected-territory expander)
