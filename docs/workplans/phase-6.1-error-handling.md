...
2. **Two-layer retry model.**
   - **Transport layer** (network errors, 429, 529, 503): handled in `get_llm()` via LangChain's `BaseChatModel.with_retry()`. Every caller gets retries automatically. Agents stay clean.
